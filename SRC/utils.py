...
def evaluate_models(X_train, y_train,X_test,y_test,models,param):
    try:
        ...
        for i in range(len(list(models))): #iterating over the models
            ...
            #remenber that we imported GridSearchCV this will help us to find the best parameters from the param defined in model_trainer.py
            gs = GridSearchCV(model,para,cv=3)
            gs.fit(X_train,y_train) #fitting the data to find the best parameters

            model.set_params(**gs.best_params_) #setting the best parameters to the model,
            #**gs.best_params_ is used to unpack the best parameters found by GridSearchCV
            model.fit(X_train,y_train) # Train model

            #predicting the train and test data to get the r2 score for each model
